...
# Fix Keras imports for compatibility
    ...
    def __init__(self, arg_init=True):
        ...
        if not arg_init:
            ...
            try:
                ...
                # Handle different model architecture formats
                if isinstance(model_architecture_json, dict):
                    # If it's already a dict, convert to JSON string
                    model_architecture_json = json.dumps(model_architecture_json)
                    
                # Fix common class name issues
                if '"class_name": "Model"' in model_architecture_json:
                    model_architecture_json = model_architecture_json.replace(
                        '"class_name": "Model"', '"class_name": "Functional"')
                ...
                try:
                    ...
                    self.monovideo = tf.keras.Model.from_config(model_config, custom_objects=objs)
...
# Compatibility setup for when imported as module
def setup_keras_compatibility():
    """Set up Keras compatibility for older model formats."""
    import tensorflow as tf
